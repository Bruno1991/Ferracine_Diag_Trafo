...
export function exportDiagnosticToExcel({
  ...
  const sheet3Data = [
    ...
    ['Carregamento de Pico por Fase', analysis.criticalPhase ? `${analysis.maxPhaseLoadingPercent}% (Fase ${analysis.criticalPhase})` : `${analysis.maxLoadingPercent}%`, `Inom = ${analysis.nominalCurrentSecondaryA} A`, analysis.loadingCondition.replace('_', ' ')],
